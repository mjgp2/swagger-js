...
var schemaToHTML = function (name, schema, models) {
  ...
  var addReference = function (schema, name, skipRef) {
    ...
    if (schema.$ref) {
      ...
      // TODO: resolve allOf?
      if ( model && model.definition && model.definition.type && model.definition.type !== "object" && model.definition.type !== "array" ) {
        skipRef = true;
      }
    ...
    if (skipRef !== true && ! _.isUndefined(model) ) {
      references[modelName] = model.definition;
    }
        ...
        if (_.isPlainObject(schema.properties)) {
          html += _.map(schema.properties, function (property, name) {
            ...
            if (property.readOnly) {
              html += ', <span class="propOptKey">read only</span>';
            }
            ...
            // inline simple type extra information
            if (property.$ref) { 
              var name = helpers.simpleRef(property.$ref);
              var model = models[name];
              if ( model && model.definition.type !== "object" ) {
                property = JSON.parse(JSON.stringify(property));
                delete property.$ref;
                _.merge(property, model.definition);
              }
            }
...
var schemaToJSON = function (schema, models, modelsToIgnore, forRequest) {
  var type = schema.type || 'object';
  var model;
  var output;

  if (!forRequest && schema.example) {
...
Model.prototype.createJSONSample = Model.prototype.getSampleValue = function (modelsToIgnore, forRequest) {
  ...
  return schemaToJSON(this.definition, this.models, modelsToIgnore, forRequest);
};
